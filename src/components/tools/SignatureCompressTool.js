...
export default function SignatureCompressTool({ defaultTarget = 20 }) {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      setProcessedStats({
        size: bestBlob.size,
        width: bestBlob.width || 0,
        height: bestBlob.height || 0
      });
    ...
    <div className={styles.container}>
      ...
      {!file && (
        <div 
          ...
          aria-label="हस्ताक्षर अपलोड करें"
        >
          <input
            type="file"
            ref={fileInputRef}
          ...
          <p className={styles.uploadText} lang="hi">अपने हस्ताक्षर (Signature) यहाँ चुनें या खींचें (Drag & Drop)</p>
          <p className={styles.uploadSubText}>केवल JPG, PNG, WebP स्वीकृत हैं</p>
          <button className="btn btn--primary" tabIndex={-1}>हस्ताक्षर की फोटो चुनें</button>
        </div>
      )}
            ...
            {processedStats && (
              <div className={styles.statsList} aria-live="polite">
                ...
                <div className={styles.actionArea} style={{ marginTop: 'var(--space-2)' }}>
                  <a 
                    href={processedUrl} 
                    download={`signature-${isCustomTarget ? customKB : targetKB}kb.jpg`}
      ...
      <div className={styles.privacyNote} role="note">
        ...
        <span lang="hi">आपके हस्ताक्षर आपके डिवाइस पर ही प्रोसेस होते हैं। फाइल सर्वर पर अपलोड नहीं होती।</span>
      </div>
    </div>
  );
}
